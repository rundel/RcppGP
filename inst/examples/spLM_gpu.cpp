...
#include "model_state_lm_gpu.hpp"
...
BEGIN_RCPP
    ...
    gpu_mat coordsD( Rcpp::as<arma::mat>(coordsD_r) );
    gpu_mat knotsD( Rcpp::as<arma::mat>(knotsD_r) );
    gpu_mat coordsKnotsD( Rcpp::as<arma::mat>(coordsKnotsD_r) );

    //arma::mat cpu_coordsD = Rcpp::as<arma::mat>(coordsD_r);
    //arma::mat cpu_knotsD = Rcpp::as<arma::mat>(knotsD_r);
    //arma::mat cpu_coordsKnotsD = Rcpp::as<arma::mat>(coordsKnotsD_r);
    ...
    model_state_lm_gpu cur_state(&cov_settings, &beta_prior, is_pp, is_mod_pp);
    ...
    //model_state_lm cpu_cur_state(&cov_settings, &beta_prior, is_pp, is_mod_pp);

    //cpu_cur_state.theta = Rcpp::as<arma::vec>(theta_settings["start"]);
    //if (!is_pp) cpu_cur_state.update_covs(cpu_coordsD);
    //else        cpu_cur_state.update_covs(cpu_knotsD, cpu_coordsKnotsD);
    
    //Rcpp::Rcout <<     cur_state.C_U.submat(0,0,4,4) << "\n";
    //Rcpp::Rcout << cpu_cur_state.C_U.submat(0,0,4,4) << "\n";

    //cpu_cur_state.update_beta(X, Y);            
    //cpu_cur_state.update_w();

    //cpu_cur_state.calc_all_loglik();


    // Initialize accept counts
    ...
    for (int s = 0; s < n_samples; s++) 
    {
        ...
        model_state_lm_gpu cand_state(cur_state);
