...
//BEGIN_RCPP
    ...
    if (!is_pp)
    {
        ...
        {              
            ...
            arma::vec j = theta_amcmc.get_jump();

            cand_state.update_theta( j );
            ...
            arma::vec alpha_theta(cov_settings.param_nfree);
            for(int i=0; i!=cov_settings.param_nfree; i++)
            {
                if (cov_settings.param_dists[i] == fixed_dist)
                    continue;

                cand_state.update_theta( j[i], i );
                cand_state.update_covs(coordsD);
                ...
                alpha_theta[i] = std::min(1.0, exp(cand_state.loglik - cur_state.loglik));
                if (Rcpp::runif(1)[0] <= alpha_theta[i])
                {
                    cur_state = cand_state;
                    //accept_theta++;
                    //batch_accept_theta++;
