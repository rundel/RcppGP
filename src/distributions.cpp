...
param_dists_map::map = boost::assign::map_list_of("fixed",         fixed_dist)
...
template<> double log_likelihood<fixed_dist>(double x, arma::vec const& hyperparams)
{
    return 0.0;
}

template<> double log_likelihood<uniform_dist>(double x, arma::vec const& hyperparams)
{
    return 0.0;
}

template<> double log_likelihood<invgamma_dist>(double x, arma::vec const& hyperparams)
{
...
template<> double log_likelihood<normal_dist>(double x, arma::vec const& hyperparams)
{
...
{
    if      (dist == fixed_dist)    return log_likelihood<fixed_dist>(x,hyperparams);
    else if (dist == uniform_dist)  return log_likelihood<uniform_dist>(x,hyperparams);
